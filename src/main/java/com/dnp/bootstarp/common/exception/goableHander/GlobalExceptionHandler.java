...
public class GlobalExceptionHandler {
    ...
    public ErrorTip unauthorizedException(Exception e) {
        return new ErrorTip(HttpStatus.FORBIDDEN.value(), "没有权限");
    }

    @ExceptionHandler(UnknownSessionException.class)
    @ResponseStatus(HttpStatus.OK)
    public ErrorTip unknownSessionException(Exception e) {
        logger.error("会话已经过期:", e.getMessage());
    ...
    public ErrorTip handleMissingServletRequestParameterException(MissingServletRequestParameterException e) {
        ...
        return new ErrorTip(HttpStatus.BAD_REQUEST.value(), "没有权限");
    }
    /**
     * 400 - Bad Request
     */
    ...
    public ErrorTip handleBindException(BindException e) {
        ...
        String code = error.getDefaultMessage();
        String message = String.format("%s:%s", field, code);
        return new ErrorTip(HttpStatus.BAD_REQUEST.value(), "参数绑定失败="+message);
    }
